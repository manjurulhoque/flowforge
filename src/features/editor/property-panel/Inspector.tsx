...
export function Inspector() {
	...
	return (
		<motion.aside
			...
			className="flex h-full w-[320px] shrink-0 flex-col border-l border-zinc-200 bg-white"
		>
					...
					>
						...
						{tab === t.id && (
							<motion.span
								...
								className="absolute inset-x-1.5 -bottom-px h-0.5 rounded-full bg-zinc-800"
							/>
						)}
					</button>
				))}
...
function EmptyInspector({ count }: { count: number }) {
	return (
		<aside className="flex h-full w-[320px] shrink-0 flex-col items-center justify-center border-l border-zinc-200 bg-white p-8 text-center">
