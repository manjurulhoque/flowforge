...
export function Pricing() {
  return (
    <section id="pricing" className="px-5 sm:px-8 py-20 sm:py-28 border-t border-[var(--border)]">
      <div className="mx-auto max-w-5xl">
        <Reveal className="max-w-xl">
          ...
          <p className="mt-4 text-[15px] text-[var(--text-secondary)]">
            Start free on a single workspace. Upgrade when the team needs
            more rooms to design in.
          </p>
        </Reveal>
          ...
          {tiers.map((t, i) => (
            <Reveal key={t.name} delay={i * 0.08} className="h-full">
              ...
              >
                ...
                <h3 className="text-[15px] font-medium text-[var(--text-primary)]">{t.name}</h3>
                <p className="mt-3 flex items-baseline gap-1.5">
                  <span className="text-3xl font-semibold text-[var(--text-primary)] font-display">
                    {t.price}
                  </span>
                  <span className="text-[12.5px] text-[var(--text-tertiary)]">{t.unit}</span>
                </p>

                <ul className="mt-6 space-y-2.5 flex-1">
                  {t.features.map((f) => (
                    <li key={f} className="flex items-start gap-2 text-[13px] text-[var(--text-secondary)]">
                      <Check size={14} className="mt-0.5 shrink-0 text-[var(--accent-strong)]" strokeWidth={2.5} />
                      {f}
                    </li>
                  ))}
                </ul>
