					...
					{nodes.map((n) => {
						...
						return (
							...
							>
								...
								{n.flagged && (
									<g
										transform={`translate(${NODE_W - 16}, -8)`}
									>
			...
			<div className="border-t border-[var(--border)] bg-[var(--bg-raised)] p-3.5 sm:p-4">
				...
				<ul className="space-y-2">
					<ValidationRow
						...
						text={
							<>
								<span className="mono text-[var(--sev-error)]">
									cache
								</span>{" "}
								has no eviction or rate-limit policy configured
							</>
						}
					/>
					<ValidationRow
...
function ValidationRow({
	...
	return (
		<li className="flex items-start gap-2 text-[12.5px] text-[var(--text-secondary)]">
			...
			<span
				className="mono text-[10.5px] uppercase tracking-wide shrink-0"
				style={{ color }}
			>
				{level}
			</span>
