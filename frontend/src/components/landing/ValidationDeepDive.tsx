...
export function ValidationDeepDive() {
  return (
    <section id="validation" className="px-5 sm:px-8 py-20 sm:py-28 border-t border-[var(--border)]">
      <div className="mx-auto max-w-5xl">
        <Reveal className="max-w-xl">
          ...
          <p className="mt-4 text-[15px] text-[var(--text-secondary)] leading-relaxed">
            Every edge and node is checked against a growing rule set for
            distributed systems &mdash; missing entry points, shared
            databases, under-replicated services &mdash; before you export
            anything.
          </p>
        </Reveal>
        ...
        <div className="mt-12 grid lg:grid-cols-2 gap-5 items-stretch">
          <Reveal className="h-full">
            <div className="h-full rounded-[var(--radius-lg)] border border-[var(--border)] bg-[var(--surface)] p-6 flex items-center justify-center">
              <svg viewBox="0 0 320 220" className="w-full max-w-xs" role="img" aria-label="Small diagram: gateway connecting to orders-api and users-api, both linked to a shared users_db">
                <line x1="60" y1="110" x2="150" y2="50" stroke="var(--border-strong)" strokeWidth="1.5" />
                <line x1="60" y1="110" x2="150" y2="170" stroke="var(--sev-error)" strokeOpacity="0.6" strokeWidth="1.5" />
                <line x1="150" y1="50" x2="250" y2="110" stroke="var(--border-strong)" strokeWidth="1.5" />
                <line x1="150" y1="170" x2="250" y2="110" stroke="var(--sev-warn)" strokeOpacity="0.7" strokeWidth="1.5" strokeDasharray="4 4" />

                <MiniNode x={20} y={92} label="gateway" Icon={Globe} />
                <MiniNode x={110} y={32} label="orders-api" Icon={Server} flagged="error" />
                <MiniNode x={110} y={152} label="notifs" Icon={Server} flagged="info" />
                <MiniNode x={210} y={92} label="users_db" Icon={Database} flagged="warn" />
              </svg>
            </div>
          </Reveal>
            ...
            <div className="h-full rounded-[var(--radius-lg)] border border-[var(--border)] bg-[var(--bg-raised)] overflow-hidden flex flex-col">
              <div className="flex items-center gap-1.5 px-4 h-10 border-b border-[var(--border)] shrink-0">
                ...
                <span className="mono text-[11px] text-[var(--text-tertiary)] ml-2">
                  flowforge validate --project checkout-platform
                </span>
              </div>
              <div className="p-5 space-y-4 mono text-[12.5px] leading-relaxed overflow-x-auto">
                {lines.map((l) => (
                  <div key={l.text}>
                    ...
                    <p className="text-[var(--text-tertiary)] pl-[3.5ch]">{l.fix}</p>
                  </div>
                ))}
                <p className="text-[var(--text-tertiary)] pt-2 border-t border-[var(--border)]">
                  3 findings &middot; 1 error &middot; 1 warning &middot; 1 info
...
function MiniNode({
  ...
  return (
    <g transform={`translate(${x}, ${y})`}>
      <rect width={96} height={34} rx={8} fill="var(--surface-2)" stroke={stroke} strokeWidth={flagged ? 1.4 : 1} />
      <foreignObject x={8} y={0} width={84} height={34}>
        <div className="h-full flex items-center gap-1.5">
          <Icon size={11} strokeWidth={2} color={flagged ? stroke : "var(--text-secondary)"} />
          <span className="mono text-[10px] text-[var(--text-primary)] truncate">{label}</span>
        </div>
      </foreignObject>
    </g>
  );
