...
/** Validation runs on the backend engine; this maps results to the editor UI. */
export const validationService = {
	async validate(
		...
		try {
			const res = await apiFetch<ValidationResultDto>("/validation", {
				method: "POST",
				auth: true,
				body: {
					nodes: nodes.map((n) => ({
			...
			return res.issues.map((issue, index) => ({
				id: `${issue.code}-${index}`,
			...
			// Backend offline — fall back to a minimal local pass so the
			// editor still surfaces obvious structural problems.
			return localValidate(nodes, edges);
		}
	},
...
/**
 * Minimal local fallback used when the backend is unreachable. Mirrors a
 * subset of the server rules: disconnected nodes, shared databases,
 * unhealthy statuses and single-replica services.
 */
function localValidate(
	...
	for (const n of nodes) {
		...
		if (n.data.category === "databases" && (indegree.get(n.id) ?? 0) > 3) {
			issues.push({
				id: uid("iss"),
				severity: "warning",
		...
		if (n.data.status === "error" || n.data.hasIssue) {
			issues.push({
				...
				affectedNodeIds: [n.id],
				suggestion: "Check the health endpoint and recent deployments.",
			});
		}
		if (n.data.category === "services" && n.data.config.replicas < 2) {
			issues.push({
				...
				affectedNodeIds: [n.id],
				suggestion: "Run at least 2 replicas for high availability.",
			});
		}
	}
	...
	if (hasService && !hasGateway) {
		issues.push({
			...
			affectedNodeIds: [],
			suggestion: "Add an API Gateway to centralize routing and auth.",
		});
	}
	return issues;
}
